var questions = [
    {
        title: "What does HTML stand for?",
        choices: ["Hyper Text Markup Language", "Home Tool Markup Language", "Hyperlinks and Text Markup Language", "History Text Markup Language"],
        answer: "Hyper Text Markup Language"
    },

    {
        title: "Who is making the Web standards?",
        choices: ["Mozilla", "Google", "The World Wide Web Consortium", "Microsoft"],
        answer: "The World Wide Web Consortium"
    },

    {
        title: "Choose the correct HTML element for the largest heading:",
        choices: ["<heading>", "<h1>", "<h6>", "<h2>"],
        answer: "<h1>"
    },

    {
        title: "What is the correct HTML element for inserting a line break?",
        choices: ["<lb>", "<break>", "<br>", "<div>"],
        answer: "<br>"
    },

    {
        title: "What does CSS stand for?",
        choices: ["Colorful Style Sheets", "Creative Style Sheets", "Computer Style Sheets", "Cascading Style Sheet"],
        answer: "Cascading Style Sheet"
    },

    {
        title: "Where in an HTML document is the correct place to refer to an external style sheet?",
        choices: ["In the <body> section", "At the end of the document", "In the <head> section", "Before the <head> section"],
        answer: "In the <head> section"
    },

    {
        title: "Which HTML attribute is used to define inline styles?",
        choices: ["font","style", "class", "styles"],
        answer: "style"
    },

    {
        title: "Which HTML attribute is used to define inline styles?",
        choices: ["bg-color", "background-color", "color", "text-color"],
        answer: "background-color"
    },

    {
        title: "Where is the correct place to insert a JavaScript?",
        choices: ["The <body> section", "The <head> section", "Both the <head> section and the <body> section are correct", "Nether <head> section and the <body> section are correct"],
        answer: "The <head> section"
    },

    {
        title: "How do you create a function in JavaScript?",
        choices: ["function = myFunction()", "function:myFunction", "function=myfunction", "function myFunction"],
        answer: "function = myFunction()"
    },
];
